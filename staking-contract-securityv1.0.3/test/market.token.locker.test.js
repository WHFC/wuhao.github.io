...
describe('MarketTokenLocker', () => {
    before(async function () {
        await deployments.fixture(['MarketTokenLocker'])
        const { deployer } = await ethers.getNamedSigners()
        const { marketReceiver } = await getNamedAccounts()
        const { signer1, signer2, signer3 } = await ethers.getNamedSigners()
        this.deployer = deployer
        this.marketReceiver = marketReceiver;
        this.signer1 = signer1;
        this.signer2 = signer2;
        this.signer3 = signer3;

        this.multiSignature = await ethers.getContract('MarketMultiSignature');
        this.tokenLocker = await ethers.getContract('MarketTokenLocker')
    ...
    it('ApplyToken', async function () {
        ...
        console.log((await this.token.balanceOf(this.marketReceiver)).toString());
        expect(await this.token.balanceOf(this.marketReceiver)).to.be.equal(this.totalRelease);
        */
    })
