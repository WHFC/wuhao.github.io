...
describe('PrivateFundTokenLocker', () => {
    before(async function () {
        await deployments.fixture(['PrivateFundTokenLocker'])
        const { deployer } = await ethers.getNamedSigners()
        const { privateFundReceiver } = await getNamedAccounts()
        const { signer1, signer2, signer3 } = await ethers.getNamedSigners()
        this.deployer = deployer
        this.privateFundReceiver = privateFundReceiver;
        this.signer1 = signer1;
        this.signer2 = signer2;
        this.signer3 = signer3;

        this.multiSignature = await ethers.getContract('PrivateFundMultiSignature');
        this.tokenLocker = await ethers.getContract('PrivateFundTokenLocker')
    ...
    it('ApplyToken', async function () {
        ...
        console.log((await this.token.balanceOf(this.privateFundReceiver)).toString());
        expect(await this.token.balanceOf(this.privateFundReceiver)).to.be.equal(this.totalRelease);
    })
