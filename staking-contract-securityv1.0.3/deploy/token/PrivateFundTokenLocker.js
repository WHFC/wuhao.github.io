...
module.exports = async function ({ ethers, getNamedAccounts, deployments, getChainId, getUnnamedAccounts }) {
  ...
  const { privateFundReceiver } = await getNamedAccounts()

  const { signer1, signer2, signer3 } = await getNamedAccounts()
  let token = await ethers.getContract('KIKIToken')
  
  await deploy('PrivateFundMultiSignature', {
    from: deployer.address,
    args: [token.address, privateFundReceiver, [signer1, signer2, signer3], 2],
    log: true,
    contract: 'MultiSignature',
  });
  let multiSignature = await ethers.getContract('PrivateFundMultiSignature');
  ...
  await deploy('PrivateFundTokenLocker', {
    from: deployer.address,
    args: ['PrivateFund TokenLocker', 'PFTK', token.address, firstLockSeconds, firstLockPercent, lockPeriod, lockPeriodNum],
    log: true,
    contract: 'TokenLocker',
  })

  let tokenLocker = await ethers.getContract('PrivateFundTokenLocker')
  /*
  ...
  if (releaseInfo.receiver != multiSignature.address) {
    ...
    tx = await tx.wait()
    console.log('create PrivateFund  tokenLocker receiver: ' + multiSignature.address + ' totalRelease: ' + totalRelease)
  }
  */
}

module.exports.tags = ['PrivateFundTokenLocker']
